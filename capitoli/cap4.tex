...
Per cercare di ridurre ulteriormente l'errore si è provato ad aumentare il range del dominio del flussi $d$ e $q$ della mappa invertita (Figure \ref{fig:errore_relativo_mappe_flusso_D_xcorrenti_ext} e \ref{fig:errore_relativo_mappe_flusso_Q_xcorrenti_ext}). Tuttavia, ciò aumcomporta un aumento significativo dell'errore per correnti vicine allo zero. La ragione di questo comportamento, come già discusso nel Paragrafo \ref{sec:limiti_dominio_flusso}, è che il codominio di corrente risultante non comprende i valori di corrente prossimi a zero. \\
Ciò risulta evidente nella Figura \ref{fig:Mappa_lambdaD_UnicoColore_dense.pdf}, dove i valori di corrente prossimi a 0 non rientrano nell'area di corrente del grafico.
...
Per cercare di ridurre l'errore si è espanso il dominio della mappa di corrente diretta anche per valori positivi di $i_d$ (Figure \ref{fig:errore_relativo_mappe_flusso_D_xcorrenti_positive} e \ref{fig:errore_relativo_mappe_flusso_Q_xcorrenti_positive}). 
In questa maniera gli estremi del dominio del flusso calcolati come esposto nel Paragrafo \ref{sec:limiti_dominio_flusso}. Come si può notare in questo modo il codominio di corrente copre anche i valori intorno allo 0 (Figura \ref{fig:Mappa_lambdaD_UnicoColore_positive}).
\begin{comment}
