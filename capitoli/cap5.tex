...
In Figura \ref{fig:Confronto_flusso_AB_algoritmo} viene riportato il confronto tra $\lambda_{\alpha\beta}$ reale e stimato tramite l'Equazione \ref{eq:integration_flux2} per la fase M1. Tale stima è stata effetutata fissando il valore di resistenza a quello nominale (2.726 $\Omega$) e senza imporre una resistenza variabile. Quindi non si è utilizzato il valore ottenuto dalla stima di resistenza. \\
Per confrontare i risultati, in Figura \ref{fig:Confronto_flusso_AB_algoritmo_res_est} viene riportato il grafico ottenuto utilizzando il valore di resistenza stimato.
\begin{figure}
\centering
    \includegraphics[height=8 cm]{immagini/procedura_misurazione/algoritmo_di_stima/Confronto_flusso_AB_algoritmo.pdf}
    \caption{Confronto tra $\lambda_{\alpha\beta}$ reale e stimato (fase M1). Il valore di resistenza usato dall'algoritmo è pari a quello reale ($\hat{R}_s$ = 2.726 $\Omega$).}
...
\centering
    ...
    \caption{Confronto tra $\lambda_{\alpha\beta}$ reale e stimato (fase M1). In questo caso si è utilizzato il valore di $R_s$ stimato ($\hat{R}_s$ = 2.7548).}
...
In Figura \ref{fig:Confronto_flusso_DQ_algoritmo} si riporta il confronto tra las stima di $\lambda_{dq}$ stimato e reale per la fase M1, fissando la resistenza stimata a quella nominale ($\hat{R}_s$ = 2.726 $\Omega$).\\
Utilizzando la stima della resistenza si ottiene il grafico riportato in Figura \ref{fig:Confronto_flusso_DQ_algoritmo}. Si può notare come la stima del flusso sia imprecisa, a casua della stima non accurata della resistenza. Tuttavia, questo problema verrà risolto in seguito con l'applicazione della formula finale dell'algoritmo. Nella stima di $\lambda_{dq}$ il risultato sarà più accurato.\\
...
\centering
    ...
    \caption{Confronto tra $\lambda_{dq}$ reale e stimato (fase M1).Si è utilizzato il valore di $R_s$ stimato ($\hat{R}_s$ = 2.7548)}
...
\centering
    ...
    \caption{Confronto tra $\lambda_{dq}$ reale e stimato (fase M1).}
