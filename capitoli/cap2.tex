\chapter{Trattazione teorica della procedura di stima}

\section{Struttura delle mappe di flusso stimato}
Le mappe di flusso stimate esprimono il flusso magnetico in funzione delle correnti $i_d$ e $i_q$ e la posizione meccanica.\\
L'area di formata dai punti di corrente $i_{dq}$ rispetto ai quali si vuole stimare il flusso è un rettangolo nel piano $dq$. Tale rettangolo deve contenere tutte i punti di interesse per l'azionamento sotto test \cite{Armando}.\\
...
Nel set-up sperimentale il motore di test (MUT) è accoppiato a un motore controllato in velocità. Tale motore trascina il motore di test imponendo una velocità di rotazione costante, l'articolo \cite{bojoi2024} propone una velocità pari a 1/3 della velocità nominale del motore di test.\\
...
Dato che che si adotta la convenzione degli a assi PM, il dominio dell'esplorazione del flusso copre il primo e il secondo quadrante del piano $i_{dq}$. Il dominio di corrente è del tipo $i_d \in [-10\,\text{A}; 10\,\text{A}]$ e $i_q \in [0\,\text{A}; 10\,\text{A}]$.\\
...
\begin{itemize}
    \item primo impulso M1 (motore): motore è controllato in catena chiusa imponendo $\bm{i}_{dq}^{M1}=[i_{d,k}\ \ \ i_{q,k}]^T$;
...
\end{itemize}
Una volta raggiunta la condizione a regime, le correnti di fase, le tensioni ai terminali del MUT e la posizione angolare meccanica vengono misurate per almeno una rotazione meccanica \cite{bojoi2024}.
...
Alternando in sequenza il funzionamento della macchina elettrica da generatore a motore è possibile sfruttare tale simmetria per eliminare la tensione associata alla resistenza statorica. \\
Se il segno della corrente $i_q$ è invertito per ottenere il vettore simmetrico rispetto all'asse $d$, ogni componente armonica del flusso mantiene la stessa ampiezza, con fase specchiata.\\ Perciò:
...
Questa proprietà verrà sfruttata in seguito per stimare il flusso,   eliminando l'errore dovuto a una stima iniziale errata della resistenza di statore.
...
Per stimare il flusso concatenato medio, si considera l'equazione di tensione a regime permanente della macchina sincrona con riferimento a un determinato punto della griglia $(i_{d,k},i_{q,k})$ \cite{Armando}:
...
Il flusso concatenato può essere quindi calcolato come segue:
...
\end{equation}
dove $v_{d,i}$ e $v_{q,i}$ rappresentano la tensione misurata media su un periodo meccanico. 
...
Tale espressione si può ricavare applicando la media integrale ad entrambi i membri dell'equazioni:
\begin{equation}
mean(\boldsymbol{\lambda}_{\alpha\beta}^{y}(t)) = mean(\boldsymbol{\lambda}_{\alpha\beta}^{y}(t=0) + \int_{t_{0}}^{t_{0}+T} (\boldsymbol{v}_{\alpha\beta}^{y} - R_{s} \cdot \boldsymbol{i}_{\alpha\beta}^{y}) dt )
\end{equation}

Dato che la media del flusso $\lambda_{\alpha\beta}$ su un periodo meccanico deve essere nulla e alla proprietà di linearità:
\begin{equation}
0 = mean(\boldsymbol{\lambda}_{\alpha\beta}^{y}(t=0)) + mean(\int_{t_{0}}^{t_{0}+T} (\boldsymbol{v}_{\alpha\beta}^{y} - R_{s} \cdot \boldsymbol{i}_{\alpha\beta}^{y}) dt)
\end{equation}

Tenenendo conto che la media integrale di un valore costante  equivale proprio a tale valore, si ottiene infine l'equazione \ref{eq:eq1}
