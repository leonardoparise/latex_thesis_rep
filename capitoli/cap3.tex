...
Tale andamento crescente si riscontra anche con il flusso $\lambda_q$ fissando la corrente $i_d$ (Figura \ref{fig:Flusso_D_vs_Iq_thetaVar}). Al contrario, il flusso $\lambda_d$ con $i_d$ fisso non presenta questa tendenza.\\
Nel caso del cross-coupling si riscontra un maggiore effetto della variazione della posizione (Figure \ref{fig:Flusso_D_vs_Id_thetaVar_iq_1}, \ref{fig:Flusso_D_vs_Iq_thetaVar}  e \ref{fig:Flusso_Q_vs_Id_thetaVar}). Nell'accoppiamento diretto invece l'effetto è meno significativo e quasi ininfluente nel caso della Figura \ref{fig:Flusso_Q_vs_Iq_thetaVar}. si può notare anche come l'andamento del flusso $\lambda_d$ rimanga invariato invertendo il segno della corrente $i_q$ (Figure \ref{fig:Flusso_D_vs_Id_thetaVar} e \ref{fig:Flusso_D_vs_Id_thetaVar_iq_1}).
...
Si può poi analizzare varia il flusso $\lambda_d$ spostandosi lungo la curva MTPA (Figure \ref{fig:Flusso_D_vs_Id_MTPA_Specifictheta} e \ref{fig:Flusso_D_vs_Iq_MTPA_Specifictheta}).
...
Si procede poi con l'analizzare l'andamento del flusso $\lambda_d$  rispetto alla corrente $i_d$ o $i_q$ nominale (Figure \ref{fig:MTPA_Flusso_D_vs_Iq_thetaVar} e \ref{fig:MTPA_Flusso_D_vs_Id_thetaVar}) mentre l'altra componente di corrente viene fatta variare.
...
\subsection{Analisi dei flussi relativi}
...
Il grafico in Figura \ref{fig:Differenza_Flusso_D_MTPA_vs_Zero} del flusso può anche essere espresso in maniera relativa, dividendo per il flusso $\lambda_d$ a corrente nulla.\\
Si è valutata anche la dipendenza dalla posizione della differenza del flusso $\lambda_d$ nel punto MTPA nominale e a corrente nulla, per ogni posizione meccanica.\\
